...
function renderExerciseTopics(){
  ...
  box.querySelectorAll("[data-excat]").forEach(b=>b.addEventListener("click",()=>{
    ...
    const start=$("#exerciseStartBtn");if(start)start.disabled=false;
    startExercises(exerciseCat);
  }));
}
...
function endExercises(){
  ...
  addXP(20);
  $("#exerciseGame").hidden=true;$("#exerciseEnd").hidden=false;$("#exerciseFinal").textContent=es+" / "+total;$("#exerciseMessage").textContent=`Siz +${es*10+20} XP oldingiz! Umumiy XP: ${xp()}`;
  updateAll()
}
