...
function finishLesson(){
  if(!cat||!L[cat])return;
  const lp=lessonProgress(cat);lp.completed=true;lp.lastIndex=L[cat].length-1;P.lastActivity={type:"lesson",cat,index:L[cat].length-1};saveProgress();
  celebrateLesson();toast("🎉 Mavzu yakunlandi!");
  setTimeout(()=>view("home"),1350);
}
...
function renderExerciseTopics(){
  ...
  box.innerHTML=exerciseTopics.map(([id,icon,title])=>`<button type="button" class="exercise-topic ${exerciseCat===id?"selected":""}" data-excat="${id}">${icon} ${title}<small>${L[id].length} ta savol</small></button>`).join("");
  box.querySelectorAll("[data-excat]").forEach(b=>b.onclick=()=>{exerciseCat=b.dataset.excat;box.querySelectorAll(".exercise-topic").forEach(x=>x.classList.toggle("selected",x===b));$("#exerciseStartBtn").disabled=false});
}
...
function saveName(){const v=$("#playerName").value.trim();if(!v)return toast("Avval ismingizni yozing 😊");P.name=v;saveProgress();$("#hello").textContent=`🌟 Salom, ${v}! Bugun nimani o'rganamiz?`;$("#nameBox")?.classList.add("saved");$("#saveName").textContent="✏️ Ismni o'zgartirish";$("#quizWelcome").textContent=`${v}, 10 ta savolga javob bering va sovrinlarni yutib oling!`;toast("Profilingiz saqlandi! 💜");updateAll()}
$("#saveName").onclick=saveName;
function initName(){const n=playerName();if(n){$("#playerName").value=n;$("#hello").textContent=`🌟 Salom, ${n}! Bugun nimani o'rganamiz?`;$("#nameBox")?.classList.add("saved");$("#saveName").textContent="✏️ Ismni o'zgartirish";$("#quizWelcome").textContent=`${n}, 10 ta savolga javob bering va sovrinlarni yutib oling!`}}
...
addEventListener("beforeinstallprompt",e=>{e.preventDefault();dp=e;$("#install").hidden=false;updateHomePWA()});$("#install").onclick=installApp;$("#installHome").onclick=installApp;$("#installCard").onclick=installApp;
...
function initDrawer(){
  ...
  document.querySelectorAll(".side-link").forEach(b=>b.addEventListener("click",()=>{hide();view(b.dataset.v)}));
}
renderCats();renderExerciseTopics();initName();initDrawer();updateAll();updateHomePWA();setTimeout(()=>$("#splash")?.classList.add("hide"),850);addEventListener("load",()=>setTimeout(()=>$("#splash")?.classList.add("hide"),300));
